...
SELECT
    ...
    SUBSTR(email,1,3) AS email_substr
FROM employees;
...
SELECT
    ...
    AVG(salary) AS average_salary,
    MAX(first_name || ' ' || last_name) FILTER (WHERE salary = (
        SELECT MAX(e2.salary)
        FROM employees e2
        WHERE e2.department = e1.department
    )) AS highest_paid_employee,
    GREATEST(AVG(salary), MIN(salary)) AS salary_comparison,
    LEAST(MAX(salary), AVG(salary)) AS another_comparison
FROM employees e1
GROUP BY department;